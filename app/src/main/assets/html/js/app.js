var World = {
	...
	createCollectionDrawable: function createCollectionDrawableFn(shelf) {
		...
		var collection = metadata.books.filter(function (entry) {
			return entry.physicalLocation.ShelfTarget === shelf
		});
		var booktobefound = metadata.books.filter(function (entry) {
			return entry.id === World.bookToFind
		});
		...
		for (let i = 0; i < metadata.books.length; i++) {
			...
			if (metadata.books[i].wikipediaUrl != "") {
				...
				wikiButtons[metadata.books[i].id] = new AR.HtmlDrawable({
					html: '<div style="width:1024px; display: table-cell; color:white; font-family:Arial, Helvetica, sans-serif; border-radius: 5%; background-color:#000000; position:relative; font-size:80px; vertical-align: middle; text-align: center;">Read more at Wikipedia</div>'
				}, 0.6, {
					viewportWidth: 1024,
					viewportHeight: 100,
					translate: {
			...
			relatedButtons[metadata.books[i].id] = new AR.HtmlDrawable({
				html: '<div style="width:450px; margin-bottom:5px; margin-top:5px; margin-left:0px; display: table-cell; color:white; font-family:Arial, Helvetica, sans-serif; border-radius: 10%; background-color:#000000; position:absolute; font-size:65px; vertical-align: middle; text-align: center; padding:15px;">Similar<br>books</div>'
			}, 0.3, {
				viewportWidth: 200,
				viewportHeight: 80,
				translate: {
			...
			if (authorCount > 1) {
				...
				morefromButtons[metadata.books[i].id] = new AR.HtmlDrawable({
					html: '<div style="width:450px; margin-bottom:5px; margin-top:5px; margin-left:500px; display: table-cell; color:white; font-family:Arial, Helvetica, sans-serif; border-radius: 10%; background-color:#000000; position:absolute; font-size:65px; vertical-align: middle; text-align: center; padding:15px;">More from<br>author</div>'
				}, 0.3, {
			...
			onImageRecognized: function (target) {
				...
				//Scans a book
					...
					if (World.bookToFind === "") {
						...
						book = metadata.books.filter(function (entry) {
	...
	otherBooks: function otherBooksFn(type) {
		...
		$(".newbook").on("click", function () {
			...
			$("#otherBooks").hide();
			$("#findARshelf").show().find(".subcontent").html('<img src="' + origBook[0].shelfImage + '"><img src="' + $(this).find("img").attr("src") + '">');
		});

		$(".subfooter").on("click", function () {
			$(this).parent().hide();

			if ($(this).parent().attr("id") == "findARshelf") {
				World.bookToFind = "";
			}
			$("#infoBox").show();
		});
...
$(document).ready(function () {
	//Get collection metadata from server.
	$.getJSON("https://jsonkeeper.com/b/1XY8", function (data) {
